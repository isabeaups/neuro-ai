...
def main():
    ...
    results_log.info(
        f"Epoch: {ARGS.epochs} || Lambda: {ARGS.heb_lamb} || LR: {ARGS.lr} || EPS: {ARGS.heb_eps} || Dim: {ARGS.heb_dim} || Dataset: {ARGS.data_name.upper()} || Learning Rule: {ARGS.heb_learn.lower().capitalize()}/{ARGS.class_learn.lower().capitalize()} || Inhibition Type: {ARGS.heb_inhib.lower().capitalize()} || Function Type: {ARGS.heb_growth.lower().capitalize()}/{ARGS.class_growth.lower().capitalize()} || Focus: {ARGS.heb_focus.lower().capitalize()}/{ARGS.class_focus.lower().capitalize()} || Experiment Type: {ARGS.experiment_type.lower().capitalize()} || Test Acc: avg = {avg_test}, var = {var_test} || Train Acc: avg = {avg_train}, var = {var_train}"
    )
...
def train_and_eval(args: Tuple) -> List[float]:
    ...
    experiment: Experiment = BaseExperiment(
        model,
        params,
        f"-{params.experiment_name}-{params.experiment_type.lower()}-{params.lr}--RHO-{params.heb_rho}-{params.heb_learn.lower()}-{params.heb_growth.lower()}-{params.heb_focus.lower()}-{params.heb_inhib.lower()}-{params.heb_lamb}---{params.class_learn.lower()}-{params.class_growth.lower()}-{params.class_focus.lower()}-{num}",
    )
