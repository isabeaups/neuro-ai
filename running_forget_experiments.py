...
growth_paramaters = {"LINEAR": 2, "SIGMOID":3, "EXPONENTIAL":4}
...
for focus in focuses:
    for key, value in growth_paramaters.items():
        for batch_size, hsize in itertools.product(batch_sizes, hidden_sizes):
            for lmbda, rho, lr in parameter_pairs:
                for (
                    ...
                    exp_name = f"focus:{focus}_K{K}_SOFTHEBB_BATCH{batch_size}_HSIZE{hsize}_{key.upper()}_{clas_growth.upper()}"
                    ...
                    try:
                        ...
                        logging.info(
                            f"Started process with PID: {process.pid} on GPU: {gpu_id} | Exp: {exp_name}"
                        )
                    except Exception as e:
                        logging.error(
                            f"Failed to start process for Exp: {exp_name}. Error: {str(e)}"
                        )
                    ...
                    if len(processes) >= max_concurrent_processes:
                        ...
                        logging.info(
                            f"Process with PID: {finished_process.pid} on GPU: {finished_gpu} completed."
                        )
                        if stderr:
                            logging.error(
                                f"Standard Error for PID {finished_process.pid}:\n{stderr}"
                            )
